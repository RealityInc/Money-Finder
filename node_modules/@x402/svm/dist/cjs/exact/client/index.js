...
var ExactSvmSchemeV1 = class {
  ...
  async createPaymentPayload(x402Version, paymentRequirements) {
    ...
    const tokenProgramAddress = mintMetadata.programAddress;
    if (tokenProgramAddress.toString() !== import_token3.TOKEN_PROGRAM_ADDRESS.toString() && tokenProgramAddress.toString() !== import_token_20224.TOKEN_2022_PROGRAM_ADDRESS.toString()) {
      throw new Error("Asset was not created by a known token program");
    }
    const [sourceATA] = await (0, import_token_20224.findAssociatedTokenPda)({
      mint: selectedV1.asset,
      owner: this.signer.address,
      tokenProgram: tokenProgramAddress
    });
    const [destinationATA] = await (0, import_token_20224.findAssociatedTokenPda)({
      mint: selectedV1.asset,
      owner: selectedV1.payTo,
      tokenProgram: tokenProgramAddress
    });
    const transferIx = (0, import_token_20224.getTransferCheckedInstruction)(
      {
        source: sourceATA,
        mint: selectedV1.asset,
        destination: destinationATA,
    ...
    const sellerMemo = selectedV1.extra?.memo;
    let memoData;
    if (sellerMemo) {
      memoData = new TextEncoder().encode(sellerMemo);
      if (memoData.byteLength > MAX_MEMO_BYTES) {
    ...
    const tx = (0, import_kit3.pipe)(
      ...
      (tx2) => (0, import_kit3.appendTransactionMessageInstructions)([transferIx, memoIx], tx2),
...
// src/exact/v1/facilitator/scheme.ts
var compiledMessageDecoder = (0, import_kit5.getCompiledTransactionMessageDecoder)();
...
function registerExactSvmScheme(client, config) {
  ...
  const v1Networks = config.networks && config.networks.length > 0 ? NETWORKS.filter((name) => {
    ...
    return config.networks.some(
      (pattern) => (0, import_utils6.networkMatchesPattern)(pattern, v2Network)
    );
  }) : NETWORKS;
