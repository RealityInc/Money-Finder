...
async function batchHandler(req,res){
  ...
  const settled=await Promise.all(unique.map(async url=>{
    try { const audit=await auditPublicUrl(url); return {ok:true,...audit,repairArtifacts:buildRepairArtifacts(audit)}; }
  ...
  return res.status(200).json({
    product:'MilliAPI AI Web Readiness Batch',version:2,checkedAt:new Date().toISOString(),requested:unique.length,succeeded:successes.length,failed:failures.length,
    summary:{...verdictCounts,attentionFirst:successes.slice(0,3).map(x=>({target:x.target,verdict:x.verdict,score:x.score,topFix:x.recommendations?.[0]?.id||null,readyToApplyRepairs:x.repairArtifacts?.readyToApply||0}))},
...
if(PAYMENT_CONFIGURED){
  ...
  app.use(ROUTE,fastUnpaidChallenge({route:ROUTE,method:'POST',amount:20000,payTo:PAY_TO,description,serviceName:'MilliAPI',tags:['ai-agents','batch','web-audit','decision-ready','repair-artifacts'],iconUrl:`${PUBLIC_ORIGIN}/icon.svg`,extensions:{...discoveryExtension}}));
