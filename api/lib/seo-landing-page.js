...
export function renderLanding(slug){
  ...
  const related=Object.entries(LANDING_PAGES).filter(([key])=>key!==slug).map(([key,value])=>`<a class="card" href="/${key}"><strong>${esc(value.h1)}</strong><span>${money(byId(value.productId).priceUsd)} per paid result</span></a>`).join('');
