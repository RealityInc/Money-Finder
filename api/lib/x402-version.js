 ...
 *
 * MilliAPI serves x402 v2 challenges. v2 renamed and reshaped the fields a client needs in
 * order to construct a payment: `amount` replaced `maxAmountRequired`, `resource` became an object
 * rather than a URL string, and networks moved to CAIP-2 identifiers (`eip155:8453`) instead of names
 * like `base`. A client that only speaks v1 therefore receives a body it cannot parse, and the only
 * thing it can do is give up — which is indistinguishable, in traffic logs, from a client that simply
 * chose not to buy.
 *
 * MilliAPI has no measurement of how much of the ecosystem is still v1, so this module does
 * two things: it serves a v1 challenge to any client that asks for one, and it reports which version
 * every challenge was served as, so the question can be answered from evidence instead of assumed.
 */
 ...
 *
 * v1 clients do not announce themselves, so the default stays v2 and v1 is served only on an explicit
 * request. `explicit` distinguishes "the client asked for this" from "the client said nothing and got
 * the default", which is the part worth logging.
 */
// Accepts either a Web `Request` or a Node-style `req`, so the same negotiation works in a Next.js
// route handler and in a serverless function without either side reimplementing it.
...
export function requestedX402Version(request) {
  const url = (() => {
    try { return new URL(request.url, 'https://placeholder.invalid'); } catch { return null; }
  })();
 ...
 *
 * MilliAPI's own offer metadata (preview, value proof, buyer flow) rides along at the top
 * level. The v1 schema ignores fields it does not know rather than rejecting them, so a strict v1
 * client still parses the challenge and a richer client can still read the extras.
 */
...
export function versionNegotiation(resourceUrl) {
  return {
    served:undefined,
