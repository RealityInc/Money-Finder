...
async function probe(surface){
  const attempt=async()=>{
    ...
    try{
      ...
      const paymentHeader=Boolean(response.headers.get('payment-required')||response.headers.get('x-payment-required'));
...
export async function runMilliSteward(){
  ...
  const surfaces=await Promise.all(SURFACES.map(probe));
