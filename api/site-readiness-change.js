...
async function changeHandler(req,res){
  ...
  try{
    const baseline=decodeBaselineToken(token); const current=await auditPublicUrl(url); const baselineHost=new URL(baseline.target).hostname.toLowerCase(); const currentHost=new URL(current.target).hostname.toLowerCase();
    ...
    return res.status(200).json({product:'MilliAPI Site Readiness Change',version:2,target:current.target,...comparison,current:{score:current.score,verdict:current.verdict,summary:current.summary,topRecommendations:(current.recommendations||[]).slice(0,3),repairArtifacts:buildRepairArtifacts(current)},nextBaselineToken:current.baselineToken,pricing:{protocol:'x402',pricePerCallUsd:0.003,currency:'USDC',network:'Base'}});
...
if(PAYMENT_CONFIGURED){
  const description='Compare a public site against a portable baseline from a prior MilliAPI readiness audit. Returns score/verdict deltas, exact machine-readable changes, and current repair artifacts without requiring an account or server-side history store.'; const tags=['ai-agents','change-detection','history','web-audit','repair-artifacts'];
  app.use(ROUTE,fastUnpaidChallenge({route:ROUTE,method:'POST',amount:3000,payTo:PAY_TO,description,serviceName:'MilliAPI',tags,iconUrl:`${PUBLIC_ORIGIN}/icon.svg`,extensions:{...discoveryExtension}}));
  app.use(ROUTE,lazyX402PaymentMiddleware({routes:{[`POST ${ROUTE}`]:{accepts:[{scheme:'exact',price:PRICE,network:NETWORK,payTo:PAY_TO}],resource:`${PUBLIC_ORIGIN}${ROUTE}`,description,mimeType:'application/json',serviceName:'MilliAPI',tags,iconUrl:`${PUBLIC_ORIGIN}/icon.svg`,extensions:{...discoveryExtension}}},network:NETWORK,serviceId:'service:site_change',priceUsd:0.003,paywallConfig:{appName:'MilliAPI'}}));
}else app.use(ROUTE,(_req,res)=>res.status(503).json({error:'x402_payment_configuration_incomplete'}));
app.post(ROUTE,changeHandler); export default app;
