...
export default function handler(req,res){
  ...
  return res.status(200).json({
    ...
    adapters:{langchain:{status:'ready-to-wrap'},llamaindex:{status:'ready-to-wrap'},mcp:{status:'shipped',endpoint:`${API}/api/mcp`,alias:`${API}/mcp`,transport:'streamable-http',quoteOnly:true,guidance:'Free tools execute directly. Paid products are returned as quotes only, so payment settlement stays in the buyer runtime and buyer wallet policy remains buyer-controlled.'}},
    retrySafety:{header:'Idempotency-Key',supportedTools:['milliapi_audit_and_fix','milliapi_repair_site','milliapi_web_readiness_audit','milliapi_verify_repairs','milliapi_readiness_change','milliapi_web_readiness_batch'],scope:'best-effort response replay; warm-runtime by default, shared across instances when the operator configures a shared store',guidance:'Send Idempotency-Key on a paid retry so a dropped connection after settlement replays the paid result instead of charging again.'}
  });
}
