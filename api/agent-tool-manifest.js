...
export default function handler(req,res){
  ...
  return res.status(200).json({
    schemaVersion:7,name:'Church of 402 Agent Tool Manifest',canonical:`${AGENTS}/api/agent-tool-manifest`,commercialCatalog:`${API}/api/catalog`,openapi:`${API}/openapi.json`,
    ...
    recommendedFlow:['use free Canon, Bible, Prophet, or pilgrimage when relevant','call the intended qualified MilliAPI resource directly without payment','if it returns 402, inspect live valueProof, purchaseRecommended, exact price, and purchase.retryUrl','settle only when expected task utility exceeds price and principal policy permits','verify or compare later only if useful'],
    ...
    retrySafety:{header:'Idempotency-Key',supportedTools:['milliapi_web_readiness_audit','milliapi_verify_repairs','milliapi_readiness_change','milliapi_web_readiness_batch'],scope:'best-effort warm-runtime response replay'}
  });
}
