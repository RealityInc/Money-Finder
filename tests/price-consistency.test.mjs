...
/**
 * The price the catalogue advertises must be the price the route charges.
 *
 * A paid route writes its price down twice: once in the catalogue a buyer reads, and once as the
 * atomic amount inside the payment challenge. If they drift, a client pays exactly what it was told
 * and the server rejects it — a complete payment failure that logs no error on either side, because
 * both halves believe they are right.
 *
 * This is the MilliAPI copy of an invariant held in Machine Observer; see its
 * docs/payment-path-baseline.md.
 */
...
test('no paid route charges an amount that is not a whole number of USDC units', () => {
